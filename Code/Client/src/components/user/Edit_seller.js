...
export function Selleredit() {
    ...
    useEffect(() => {
        fetch("http://localhost:3456/editoneseller/" + seller_id)
            .then(data => data.json())
            .then((response) => {
    ...
    function handleupdate(event) {
        ...
        if (!fname) {
            ...
            axios.put(`http://localhost:3456/updatesellerdata/${seller_id}`, updatedetails)
            .then(response => {
      ...
      //JS Function to delete a seller by using the id -- START
      const [deletionStatus, setDeletionStatus] = useState('');

      const handleDeleteSeller = async () => {
        try {
          const response = await fetch(`http://localhost:3456/deleteseller/${seller_id}`, {
            method: "DELETE",
          });
      ...
      //JS Function to delete a seller by using the id -- END
            ...
            <section className="container-fluid edit_profile bg-dark d-flex align-items-center">
                <div>
                    <form className="row fs-5 d-flex align-items-end justify-content-center" onSubmit={handleupdate}>
                        ...
                        <div className="my-3 col-10 col-md-5 d-flex justify-content-evenly">
                        <button type="submit" className="btn btn-success fs-5 border-0 my-2 ps-3 py-2 rounded-3">Update</button>
                        <button onClick={handleDeleteSeller} className="btn btn-danger fs-5 border-0 my-2 ps-3 py-2 rounded-3">Delete account</button>
                        </div>
                    </form>
                </div>
            </section>
